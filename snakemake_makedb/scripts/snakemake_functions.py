...
def read_samples_CSV_classifier(spls):
...
def split_samplesCSV_classifier(PATH_ls,SAMPLE_ls,REF_Genome_ls,FILENAME_ls,OUTGROUP_ls):
...
def cp_append_files(paths,sample,filenames):
    ...
    if len(paths)>1 and len(filenames)==1:
    ...
    elif len(filenames)>1 and len(paths)>1:
        for file in filenames:
            [fwd_file, rev_file]=findfastqfile(paths[0],sample, file)
            fwd_list=fwd_list+ ' ' + fwd_file
            rev_list=rev_list+ ' ' + rev_file
            print(rev_list)
            print(fwd_list)
        ...
        raise IOError('Input will only take multiple paths OR multiple filenames, not both/neither!')
