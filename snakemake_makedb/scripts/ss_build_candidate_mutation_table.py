...
# path_to_sample_names_file="string_sampleID_names.txt"
# path_to_sample_names_file_newline="string_sampleID_names_newline.txt"
# path_to_outgroup_boolean_file="string_outgroup_bool.txt"
# path_to_outgroup_boolean_file="string_outgroup_bool_newline.txt"
#%%
''' load libraries '''
import numpy as np
import pickle
...
def main(path_to_p_file, path_to_sample_names_file, path_to_outgroup_boolean_file, path_to_list_of_quals_files, path_to_list_of_diversity_files, path_to_candidate_mutation_table, flag_cov_raw_sparse_matrix,flag_cov_norm_sparse_scale_matrix):
    ...
    for i in range(numSamples):
        ...
        with open(paths_to_quals_files[i],"rb") as f:
    ...
    with open(fname,'r') as f:
        paths_to_diversity_files=f.readlines()
    #Load in first diversity to get some stats
    with open(paths_to_diversity_files[1],'rb') as f:
        data = pickle.load(f)
    size=np.shape(data)
    GenomeLength=size[1]
    ...
    for i in range(numSamples):
        ...
        with open(paths_to_diversity_files[i],'rb') as f:
            data = pickle.load(f)
        counts[:,:,i]=data[0:8,p]
        ...
        indel_counter[:,:,i]=data[8:10,p] # Added 2019.02.12 reads supporting indels and reads supporting deletions
