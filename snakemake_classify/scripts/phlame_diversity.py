...
path_to_pos_file='chrpositions.txt'
...
def pileup2diversity(input_pileup, path_to_pos=False, path_to_ref=False):
    """Grabs relevant allele info from mpileupfile and stores as a nice array.
    Written for Phlame 11.5.22

    Args:
        input_pileup (str): Path to input pileup file.
        path_to_pos (str): Path to samtools-compatible positions file
    ...
    if path_to_ref:
        ...
        chr_starts,genome_length,scaf_names = chrpos_stats(path_to_pos)
        
    #format [A T C G a t c g]
    data = np.zeros((genome_length,num_fields)) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('-p', dest='pos', type=str, help='Path to samtools compatible positions file',required=False)
    ...
    if args.pos:
        diversity_arr, coverage_arr = pileup2diversity(args.input,pos2grab=args.pos)
        
    elif args.ref and not args.pos:
        diversity_arr, coverage_arr = pileup2diversity(args.input,path_to_ref=args.ref)
    else:
        raise IOError('Must specify either a reference genome file or samtools-compatible positions file!')
